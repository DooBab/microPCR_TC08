...
void CMicroPCRDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_PROGRESS_CHAMBER_TEMP2, m_cProgressBar2);
	DDX_Text(pDX, IDC_EDIT_CHAMBER_TEMP2, m_cTemperature2);
}
...
BOOL CMicroPCRDlg::OnInitDialog()
{
	...
	if(open_tc08_driver())
	{
		usb_tc08_set_mains(hTC08[0], 0);
		set_tc08_channels();
		long minimum_interval = usb_tc08_get_minimum_interval_ms(hTC08[0]);
		usb_tc08_run(hTC08[0], minimum_interval);
	}

	if(isConnected && tc08_connected)
...
static const int PID_TABLE_COLUMN_WIDTHS[6] = { 60, 90, 90, 80, 80, 80 };
...
void CMicroPCRDlg::OnBnClickedButtonConstants()
{
	...
	else
	{
		...
		// openConstants
		if( isTempGraphOn )
			SetWindowPos(NULL, 100, h/3, 1104, 375, SWP_NOZORDER);
		else
			SetWindowPos(NULL, w/5, h/3, 1104, 375, SWP_NOZORDER);
...
void CMicroPCRDlg::OnBnClickedButtonPcrRecord()
{
	if( !isRecording )
	{
		...
		m_recFile.WriteString(L"Number	Time	MicroPCR	Thermocouple\n");
...
LRESULT CMicroPCRDlg::OnmmTimer(WPARAM wParam, LPARAM lParam)
{
	...
	*/
	...
	// SIRI 15-10-01 TC-08 Thermocouple reader
	if(tc08_found > 0)
	{
		usb_tc08_get_temp(hTC08[0], &tc08_buffer, &times_buffer, 9, &overflow, 6, USBTC08_UNITS_CENTIGRADE, 0);
	}
	...
	////// SIRI 15-10-01 TC-08 Thermocouple reader
	CString tempStr2;
	tempStr2.Format(L"%3.3f", tc08_buffer);
	SetDlgItemText(IDC_EDIT_CHAMBER_TEMP2, tempStr2);
	m_cProgressBar2.SetPos((int)tc08_buffer);


	// 현재 사용중인 PID 값을 보여준다.
	CString pidstr;
	pidstr.Format(L"%.f", m_kp);
	SetDlgItemText(IDC_EDIT_CURRENT_P, pidstr);
	pidstr.Format(L"%.4f", m_ki);
	SetDlgItemText(IDC_EDIT_CURRENT_I, pidstr);
	pidstr.Format(L"%.0f", m_kd);
	SetDlgItemText(IDC_EDIT_CURRENT_D, pidstr);

	if( isTempGraphOn && isStarted )
		tempGraphDlg.addData(currentTemp, tc08_buffer);
	...
	{
		m_recordingCount++;
		out.Format(L"%6d	%8.0f	%3.1f	%3.3f\n", m_recordingCount, (double)(timeGetTime()-m_recStartTime), currentTemp, tc08_buffer);
...
void CMicroPCRDlg::OnBnClickedCheckTempGraph()
{
	...
	if( isTempGraphOn ){
		if( openConstants )
			SetWindowPos(NULL, 100, GetSystemMetrics(SM_CYSCREEN)/3, 1104, 375, SWP_NOZORDER);
...
// SIRI 15-10-01 TC-08 Thermocouple Reader
bool CMicroPCRDlg::open_tc08_driver (void)
{
	short hTemp;
	...
	do
	{
		...
		if (hTemp > 0)
			...
			AfxMessageBox (L"open_tc08_driver() Error");

	} while (hTemp && tc08_found < 4);
	
	if (!tc08_found)
	{
		//AfxMessageBox (L"No USB TC-08's Found");
		SetDlgItemText(IDC_EDIT_DEVICE_STATUS2, L"Disconnected");
		tc08_connected = false;
		return false;
	}
	else
	{
		SetDlgItemText(IDC_EDIT_DEVICE_STATUS2, L"Connected");
		tc08_connected = true;
		return true;
	}
}

void CMicroPCRDlg::set_tc08_channels (void)
{
	usb_tc08_set_channel(hTC08[0], 6, TC_TYPE_K);
}

void CMicroPCRDlg::OnClose()
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	usb_tc08_stop(hTC08[0]);
	CDialog::OnClose();
}
